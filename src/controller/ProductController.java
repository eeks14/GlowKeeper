...
public class ProductController {
    ...
    public void bubbleSortByExpiry(ArrayList<Product> list) {
        int n = list.size();
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - i - 1; j++) {
                if (list.get(j).getExpiryYear() > list.get(j + 1).getExpiryYear()) {
    ...
    public void bubbleSortByName(ArrayList<Product> list) {
        int n = list.size();

        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - i - 1; j++) {
                if (list.get(j).getProductName()
                    .compareToIgnoreCase(list.get(j + 1).getProductName()) > 0) {
